...
if(working.study.cohort.type=="VaccinatedCohorts"){
  # link to the db
cohortTableExposures_db<-cohortTableExposuresVax_db %>% 
                       filter(cohort_definition_id==!!study.cohorts$id[i])
} else if (working.study.cohort.type=="CovidCohorts"){
  cohortTableExposures_db<-cohortTableExposuresCov_db %>% 
                       filter(cohort_definition_id==!!study.cohorts$id[i])
} else { #gpop
  cohortTableExposures_db<-cohortTableExposuresGpop_db %>% 
                       filter(cohort_definition_id==!!study.cohorts$id[i])
}  
# cohortTableExposures_db %>% tally()

# take sample of pop -----
if(run.analysis.with.sampling==TRUE){
cohortTableExposures_db<-cohortTableExposures_db %>% 
    head(50000)
}
# cohortTableExposures_db %>% tally()
...
# age groups ----
Pop<-Pop %>% 
  mutate(age_gr=ifelse(age>=18 &  age<=44,  "18-44",
  ...
  mutate(age_gr= factor(age_gr, 
                   levels = c("18-44","45-54", "55-64",
                              "65-74", "75-84",">=85")))
# table(Pop$age_gr, useNA = "always")

# wider age groups
Pop<-Pop %>% 
  mutate(age_gr2=ifelse(age>=18 &  age<=44,  "18-44",
...
Pop<-Pop %>% 
  mutate(age_gr3=  ifelse(age>=18 &  age<=29,  "18-29",
...
MEDEA <- MEDEA %>%
  mutate(deprivation = ifelse (str_detect(value_as_string, "1"), "G1",
                       ifelse(str_detect(value_as_string, "2"), "G2",
                       ifelse(str_detect(value_as_string, "3"), "G3",
                       ifelse(str_detect(value_as_string, "4"), "G4",
                       ifelse(str_detect(value_as_string, "5"), "G5",
...
Pop<-Pop %>% 
  left_join(MEDEA %>% 
              select(person_id, deprivation) %>% 
              rename(medea=deprivation)  )
rm(MEDEA)
} else {
Pop$medea<-NA  
}
...
data.frame(Overall=t(working.data %>% 
  ...
  medea.G1=paste0(nice.num.count(sum(medea=="G1", na.rm = T)),
                    " (",  nice.num((sum(medea=="G1", na.rm = T)/length(person_id))*100),"%)"),
  medea.G2=paste0(nice.num.count(sum(medea=="G2", na.rm = T)),
                    " (",  nice.num((sum(medea=="G2", na.rm = T)/length(person_id))*100),"%)"),
  medea.G3=paste0(nice.num.count(sum(medea=="G3", na.rm = T)),
                    " (",  nice.num((sum(medea=="G3", na.rm = T)/length(person_id))*100),"%)"),
  medea.G4=paste0(nice.num.count(sum(medea=="G4", na.rm = T)),
                    " (",  nice.num((sum(medea=="G4", na.rm = T)/length(person_id))*100),"%)"),
  medea.G5=paste0(nice.num.count(sum(medea=="G5", na.rm = T)),
                    " (",  nice.num((sum(medea=="G5", na.rm = T)/length(person_id))*100),"%)"),
  medea.Missing=paste0(nice.num.count(sum(medea=="Missing", na.rm = T)),
                    " (",  nice.num((sum(medea=="Missing", na.rm =T)/length(person_id))*100),"%)")
  ))),
  ...
  if(str_detect(working.study.cohort, "21d")){
    f_u.outcome<-working.outcomes %>%  
      inner_join(working.Pop %>% 
                   ...
                   rename("Pop_cohort_start_date"="cohort_start_date"))  %>% 
      filter(cohort_start_date> Pop_cohort_start_date) %>% 
      filter(cohort_start_date<= (Pop_cohort_start_date+days(21))) }#REVISAR
  if(str_detect(working.study.cohort, "90d")){
    f_u.outcome<-working.outcomes %>%  
    inner_join(working.Pop %>% 
                 select(person_id,cohort_start_date) %>% 
                 rename("subject_id"="person_id") %>% 
                 rename("Pop_cohort_start_date"="cohort_start_date"))  %>% 
    filter(cohort_start_date> Pop_cohort_start_date) %>% 
    filter(cohort_start_date<= (Pop_cohort_start_date+days(90))) #REVISAR
}
}
...
events_table <- rbind(events_table, 
                      c(nrow(f_u.outcome), 
                        working.study.cohort, working.outcome.name, setting="All"))

##hospitalations at time of event for sidiap
if(db.name=="SIDIAP"){ 
# hospitalisations of covid cases, from study start onwards
working.date<-as.Date("01/01/2017", "%d/%m/%Y")
hospitalisations<- cohortTableExposures_db %>% 
  select(subject_id) %>% 
  distinct() %>% 
  rename("person_id"="subject_id") %>% 
  inner_join(
...
index_hosptialisation<-hospitalisations %>%
  mutate(dtime=as.numeric(difftime(f_u.outcome_date, visit_start_date, units="days"))) %>%
  filter(dtime >= 0)%>%
  filter(dtime <= 3) %>%
  select(-dtime) %>%
  arrange(subject_id, visit_start_date) 

if(nrow(index_hosptialisation)>=1){ 
  index_hosptialisation<-index_hosptialisation%>%
    group_by(subject_id)%>%
    mutate(seq=1:length(subject_id)) %>%
    filter(seq==1) %>%
    select(subject_id, f_u.outcome_date,visit_start_date) %>% # date of admission is index date
    rename("COVID19_hospital_date"="visit_start_date")
}

events_table <- rbind(events_table, 
                      c(nrow(index_hosptialisation), 
                        working.study.cohort, working.outcome.name, setting="Hospital"))
}
...
if(working.study.cohort.type=="CovidCohorts"){#REVISAR
  if(str_detect(working.study.cohort, "21d")){
    working.Pop<-working.Pop %>%
      mutate(f_u.outcome_date=if_else(f_u.outcome==1,
                                      f_u.outcome_date, 
...
# CovidCohorts: splitting follow up time
if(working.study.cohort.type== "CovidCohorts"){
  if(str_detect(working.study.cohort, "21d")){
    a<- survSplit(Surv(f_u.outcome.days, f_u.outcome) ~., 
                  working.Pop,
...
if(length(unique(working.data$gender))>1){
...
if(length(unique(working.data$cond.comp))>1){
...
if(length(unique(working.data$drug.comp))>1){
...
if(length(unique(working.data$cond.drug.comp))>1){
...
if(working.study.cohort.type== "VaccinatedCohorts"){
  if(str_detect(working.study.cohort, "BNT162b2 first-dose")|
     (str_detect(working.study.cohort, "ChAdOx1 first-dose"))|
      (str_detect(working.study.cohort, "mRNA-1273 first-dose"))|
     (str_detect(working.study.cohort, "BNT162b2 second-dose"))){ #Berta
