...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= (P.START_DATE + -365*INTERVAL'1 day') AND A.END_DATE >= (P.START_DATE + 0*INTERVAL'1 day') AND A.END_DATE <= P.OP_END_DATE ) cc 
...
(
  ...
  (
    ...
    (
      --cteEndDates
      select PERSON_ID, (EVENT_DATE + -1 * 56*INTERVAL'1 day') as END_DATE -- unpad the end date by 56
      FROM
      (
				...
				(
					...
					select PERSON_ID, (DRUG_EXPOSURE_END_DATE + 56*INTERVAL'1 day'), 1 as EVENT_TYPE, NULL
					FROM drugTarget D
				) RAWDATA
      ) E
